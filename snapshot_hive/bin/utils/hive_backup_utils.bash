...
get_table_relative_location() {
  ...
  if ! table_absolute_path=$( ${BEELINE} -e "show create table $1.$2 ${TERMINATE}" | \
    grep -E "^LOCATION$" -A 1 |  grep -E -v  "^LOCATION$" | tr -d "'" | tr -d " "); then
...
backup_table_schema () {
  ...
  info "extracting schema for table ${hive_db_name}.${hive_table_name} "
  ...
  "show create table ${hive_db_name}.${hive_table_name} ${TERMINATE}" \
   >>  "${__project_dir}/output/${SCHEMA_DDL_FILE}" ;then
    { error "ERROR while extracting schema of table ${hive_db_name}.${hive_table_name}" ; exit 1 ;}
  fi

  echo "${TERMINATE}" >>  "${__project_dir}/output/${SCHEMA_DDL_FILE}"
  ...
  if listpartitions=$( ${BEELINE} -e "show partitions ${hive_db_name}.${hive_table_name} ;" 2> /dev/null ) \
    ...
    info "table ${hive_table_name} is a partionned table"
    info "Extracting Partition DDL"
    ...
    for tablepart in ${listpartitions}
       do
          ...
          echo "ALTER TABLE ${hive_table_name} ADD PARTITION ($partname\");" \
           >> "${__project_dir}"/output/"${SCHEMA_DDL_FILE}"
       done
  fi
  info "schema of table ${hive_db_name}.${hive_table_name} successfully extracted in ${__project_dir}/output/${SCHEMA_DDL_FILE}"
...
backup_table_data() {
  ...
  info "Taking a snapshot of the table ${hive_db_name}.${hive_table_name}"

  table_absolute_path=$(cat "${__project_dir}/output/${SCHEMA_DDL_FILE}" | grep -E "^LOCATION$" -A 1 |  grep -E -v  "^LOCATION$" | tr -d "'" |  tr -d " " )
  table_relative_path=$(echo "${table_absolute_path}" | sed -E 's#hdfs://([^/]+)*##')
  # create hdfs snapshot for the table
  if ! "${__project_dir}/../snapshot_hdfs/bin/core/hdfs_backup_user_operations.bash" create_snapshot "${table_relative_path}" "${SNAPSHOT_NAME}" ; then
    { error "ERROR while taking  snapshot for table ${hive_db_name}.${hive_table_name} " ; exit 1 ; }
  fi
...
schema_table_check_and_apply_retention() {
  ...
  info "number of DDL snapshots to retain not set, applying default retention=${DEFAULT_NB_DDL_SNAPSHOTS}"

  ## list of existing schema snapshots for a table in a chronological order
  local arr_existing_DDL_snapshots=( $(ls -tr  "${__project_dir}/output/" | grep -E "^${hive_db_name}.${hive_table_name}(.+).hql$" ) )
  ...
  if [[ ${nb_existing_DDL_snapshots} -gt ${nb_DDL_snapshots_to_retain} ]]; then
    ...
    for DDL_snap_to_remove in "${arr_DDL_snapshots_to_remove[@]}"; do
      rm -f  "${__project_dir}/output/${DDL_snap_to_remove}"
    done

  else
    info "no additional DDL snapnshots to remove, ${nb_existing_DDL_snapshots} DDL snapnshots exists for table ${hive_db_name}.${hive_table_name}  "
  fi

  info "list of remaining DDL snapshots"
  echo "$(ls ${__project_dir}/output/ | egrep "^${hive_db_name}.${hive_table_name}(.+).hql$"  | xargs -I {} ls   ${__project_dir}/output/{} | sort -h ) "
...
data_table_check_and_apply_retention(){
  ...
  "${__project_dir}/../snapshot_hdfs/bin/core/hdfs_backup_user_operations.bash" \
...
backup_table() {
  ...
  info "backup table $1 $2 finished successfully"
}
